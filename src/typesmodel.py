...
_types_nt_fname = "typesdb_nt.py"
_types_fname = "typesdb.ral"
typesdb = "typesdb"

def snake(sdelim: str) -> str:
    """
...
class TypesModel:
    ...
    A user types_domain can then be populated into this schema.
    """
    ...
    # The user does not suppy the types_domain types_domain, so we can assume it is located within
    # our module as indicated.
    types_pkg = types_home

    # Subsystem class types_domain files
    ...
    types_subsystem = {}  # Parsed subsystems, keyed by subsystem file name
    ...
    def create_db(cls):
        """
        Create a types_domain database in PyRAL complete with relvars and constraints
        from a parse of the types_domain xcm file
        """
        # Create a TclRAL session with an empty tclral
        cls.db = Database.open_session(typesdb)

        # Parse each metamodel file and add each class to the tclral
        for subsys_cm_file in cls.types_pkg.glob("*.xcm"):
        ...
        for sname, subsys in cls.types_subsystem.items():
            for r in subsys.rels:
                cls.add_rel(r)
        Database.names(db=typesdb)  # Log all created relvar names
        Database.constraint_names(db=typesdb)  # Log all created constraints
        Database.save(db=typesdb, fname = str(Path.cwd() / _types_fname))

        # Output a text file of the typesdb schema
        typesdb_printout = f"typesdb.txt"
        with open(typesdb_printout, 'w') as f:
            with redirect_stdout(f):
                Relvar.printall(db=typesdb)

    @classmethod
    def parse(cls, cm_path):
        """
        Parse the types_domain types_domain
    ...
    def add_class(cls, mm_class):
        ...
        Relvar.create_relvar(db=typesdb, name=cname, attrs=attrs, ids=ids)
    ...
    def add_association(cls, association):
        ...
        referring_class = snake(source['class'])
        referring_attrs = [snake(a) for a in source['attrs']]

        target = association['ref1']['target']
        if target['class'] not in cls.schema_classes:
            cls._logger.warning(f"Rel {rnum} to imported class skipped")
            return  # We'll add the association after all imported classes are resolved
        referenced_class = snake(target['class'])
        referenced_attrs = [snake(a) for a in target['attrs']]
        ...
        Relvar.create_association(db=typesdb, name=rnum,
    ...
    def add_associative(cls, associative_rel):
        ...
        if associative_rel['assoc_cname'] not in cls.schema_classes:
            cls._logger.warning(f"Rel {rnum} to imported class skipped")
            return  # We'll add the association after all imported classes are resolved
        assoc_class = snake(associative_rel['assoc_cname'])

        ref1_source = associative_rel['ref1']['source']
        ref1_from_attrs = [snake(a) for a in ref1_source['attrs']]

        target1 = associative_rel['ref1']['target']
        if target1['class'] not in cls.schema_classes:
            cls._logger.warning(f"Rel {rnum} to imported class skipped")
            return  # We'll add the association after all imported classes are resolved
        ref1_class = snake(target1['class'])
        ref1_to_attrs = [snake(a) for a in target1['attrs']]

        ref2_source = associative_rel['ref2']['source']
        ref2_from_attrs = [snake(a) for a in ref2_source['attrs']]

        target2 = associative_rel['ref2']['target']
        if target2['class'] not in cls.schema_classes:
            cls._logger.warning(f"Rel {rnum} to imported class skipped")
            return  # We'll add the association after all imported classes are resolved
        ref2_class = snake(target2['class'])
        ref2_to_attrs = [snake(a) for a in target2['attrs']]
        ...
        Relvar.create_correlation(db=typesdb, name=rnum, correlation_relvar=assoc_class,
    ...
    def add_generalization(cls, generalization):
        ...
        superclass_attrs = [snake(a) for a in genref['target']['attrs']]
        if len(generalization['genrefs']) == 1:  # All superclass refs identical
            subclass_attrs = [snake(a) for a in genref['source']['attrs']]
            subclasses = {snake(s): subclass_attrs for s in subclass_names}
        else:
            subclasses = {}
            for g in generalization['genrefs']:
                referring_subclass_name = snake(g['source']['class'])
                subclasses[referring_subclass_name] = [snake(a) for a in g['source']['attrs']]

        Relvar.create_partition(db=typesdb, name=rnum, superclass_name=snake(superclass_name),
